...
# 3. Fetch current quota limit from GCP Cloud Quotas API via gcloud
echo "[QUERY] Fetching quota details for '${QUOTA_ID_C3_VCPU}'..."

QUOTA_INFO=$(gcloud alpha cloud-quotas quota-infos describe "${QUOTA_ID_C3_VCPU}" \
  --project="${PROJECT_ID}" \
...
if [[ -z "$QUOTA_INFO" ]]; then
  echo "[ABORT] Could not retrieve current quota info. Check gcloud permissions or authentication."
  exit 1
fi
...
# 6. Submit quota preference request if flag is enabled
if [[ "$REQUEST_QUOTA_INCREASE" == "true" ]]; then
  ...
  gcloud alpha cloud-quotas quota-preferences create "${PREFERENCE_ID}" \
    --project="${PROJECT_ID}" \
    --location="global" \
    --service="${SERVICE_NAME}" \
    --quota-id="${QUOTA_ID_C3_VCPU}" \
    --preferred-value="${REQUIRED_CPUS}" \
    --dimensions="region=${REGION}" \
    --contact-email="${CONTACT_EMAIL}"

  echo "[SUCCESS] Quota increase request created successfully!"
else
